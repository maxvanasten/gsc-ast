	...
	while (index < tokens.length) {
		...
		switch (tokens[index].identifier) {
			...
			case '"': {
				...
				// TODO: I think this is positioned wrong, it should not only happen with strings but after every check.
			...
			case "=": {
				...
				if (tokens[index - 1].identifier == "identifier") {
					const variable_name: string = tokens[index - 1].content || "undefined";
			...
			case "identifier": {
				if (isNaN(+tokens[index].content)) {
					...
					output.push({
						...
						content: tokens[index].content.trim()
					})
				} else {
					// Number
